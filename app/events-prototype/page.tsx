  ...
  return (
    ...
    >
      <div className="eh-ctx-inner">
        ...
        <a className="eh-ctx-link" href={categoryMeta[category].href} style={{ color: accent }}>
          Open the full {categoryMeta[category].label.toLowerCase()} page →
        </a>
      </div>
    </section>
  );
...
export default function EventsPrototypePage() {
  ...
  if (selected) {
    const related = [...events, ...past].filter((e) => e.id !== selected.id).slice(0, 6);
    const DetailComp = selected.category === "performance" ? PerformanceDetail : ArchiveDetail;
    return (
      <DetailComp event={selected} related={related} onBack={() => setSelectedId(null)} onOpen={open} />
    );
  }
  ...
  return (
    <style>{`
      ...
      .eh-hero-content {
        ...
        padding: clamp(5rem, 11vw, 8rem) clamp(1.5rem, 6vw, 5rem) clamp(3.5rem, 7vw, 6rem);
      }
      .eh-breadcrumb {
      ...
      .eh-hero-sub {
        ...
        color: rgba(255,255,255,0.68); line-height: 1.65; max-width: 540px; margin: 0 0 2rem;
      }
      .eh-hero-cats { display: flex; flex-wrap: wrap; gap: 0.75rem; }
      .eh-hero-cat {
      ...
      .eh-hero-note { font-family: var(--font-dm-sans), sans-serif; font-size: 0.82rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; margin: 0; }
      .eh-hero-strip { display: flex; flex-wrap: wrap; gap: 0 2rem; }
      ...
      .eh-hero-pills { display: flex; flex-wrap: wrap; gap: 0.6rem; }
