// app/journey-card-mockup/v14/JourneyCardV14.tsx
// ⚠️  MOCKUP ONLY — no live data, no auth, no backend.
//
// v14 — adaptive + polish pass (builds on v13):
//   • SLOVAKIA 2026 label is now DAT Blue (#2493A9 / C.teal) everywhere.
//     The chapter location (BRATISLAVA, KOŠICE, …) stays DAT pink.
//   • Cover "DRAMATIC ADVENTURE THEATRE" link is now DAT Pink (C.pink),
//     not DAT Gold — still pointing to "/".
//   • Drama clubs render as proper logo stamps via the canonical
//     <DramaClubBadge> component (solid colored circle + white club name).
//     No more photo avatars in the chapter mini-cards.
//   • Mobile cover restores the "This is my journey." pink-leader line and
//     bumps the PASSAGE wordmark from ~26px → ~58px so the lockup feels
//     dominant, the way it does on desktop.
//   • Mobile back cover stamp grows from 132px → ~200px and is allowed to
//     overlap the surrounding mosaic squares, so it reads as stamped onto
//     the artifact, not boxed inside a clear zone.
//
// ── Adaptive sizing strategy ──
// The card is designed at a native landscape size of 1080×580. Below a
// portrait breakpoint we reflow into a passport-book (vertical) layout,
// because a wide 1080 landscape simply does not work on a phone in portrait.
// Everywhere else we keep the native landscape layout and fluidly *scale*
// it (CSS transform) so it always feels intentional — never cramped, never
// floating in dead space, never awkwardly tiny on large screens.
//
//   • Phones (vw < 760)          → passport-book reflow.
//                                  Card width ≈ clamp(300, vw - 24, 460);
//                                  height ≈ width * 1.58 (≈5:8 passport).
//   • Tablets / laptops / desktop → native 1080×580 landscape, scaled by
//                                  CSS transform with factor =
//                                    clamp(0.7, (containerW - 32) / 1080, 1.28).
//                                  Outer wrapper height is recalculated so
//                                  the rest of the page reflows around the
//                                  visible (scaled) card, not the native one.
//   • Ultra-wide (vw ≥ ~1500)    → cap scale at ~1.28 so the card has real
//                                  presence on big displays without losing
//                                  its handmade/artifact quality.
//
// Text, photos, and components inside the landscape layout scale together,
// so the design stays in proportion at every breakpoint.
...
const C = {
  ...
  ink:    "#241123",
  yellow: "#f5c842",
  gold:   "#D9A919", // DAT gold — kept in palette for parity with V13; not used in V14.
  teal:   "#2493a9",
...
// Drama clubs across the rest of the app are stamped using the canonical
// <DramaClubBadge> component (lib/dramaClubMap.ts records + a solid colored
// circle with the club name in white Anton). V14 follows that pattern instead
// of a photo avatar. `shortName` keeps the stamp readable when the full name
// is long; `primaryColor` is optional — if omitted the badge picks a stable
// color from the club name.
type DramaClub = {
  name:         string;
  shortName?:   string;
  slug:         string;
...
function SoundToggle({ playing, onToggle }: { playing: boolean; onToggle: () => void }) {
  return (
    <button type="button" onClick={onToggle} className="jc-v14-btn"
...
function ShareButton({ title, text, onCopied }: { title: string; text: string; onCopied: () => void }) {
  ...
  return (
    <button type="button" onClick={share} className="jc-v14-btn" title="Share this journey"
      style={{
        display: "flex", alignItems: "center", gap: 7,
...
// ── Drama club feature: logo-stamp + horizontal mini-card ────────────────────
// The avatar uses the canonical <DramaClubBadge> (solid colored circle with
// the club name set in white Anton), matching how drama clubs are stamped
// across the rest of the app (DramaClubPageTemplate, productions, events).
// If a club record provides `primaryColor` we use it; otherwise the badge
// derives a stable color from the club name.
function DramaClubFeature({ club }: { club: DramaClub }) {
  const stampName = club.shortName ?? club.name;
  return (
    <a
      href={`/drama-club/${club.slug}`}
      className="jc-v14-club"
      style={{
    ...
    >
      <div
        style={{
          ...
          filter: "drop-shadow(0 1px 2px rgba(36,17,35,0.18))",
        }}
        aria-hidden
      >
        <DramaClubBadge
...
// V14 introduces an "xl-mobile" size for the mobile cover so the wordmark
// reads with the same editorial impact it has on landscape, and a separate
// "sm" for the compact chapter mini-header on mobile (kept smaller because
// it sits inside the pinned header). Sizes use clamp() where the value will
// be read by phones, so PASSAGE scales smoothly from narrow → large phones.
function PassageStack({
  size = "lg", align = "left",
}: {
  size?:  "lg" | "md" | "sm" | "lg-mobile" | "xl-mobile";
  align?: "left" | "center";
}) {
  let big: number | string;
  let small: number | string;
  let bigMargin = 6;
  switch (size) {
    case "lg":
      big = 80; small = 17; break;
    case "md":
      big = 26; small = 11; break;
    case "sm":
      big = 22; small = 10; bigMargin = 4; break;
    case "lg-mobile":
      // Mobile chapter mini-header — bigger than desktop md so PASSAGE still
      // reads as the wordmark, not a label. Fluid through phone widths.
      big   = "clamp(34px, 9.5vw, 42px)";
      small = "clamp(11px, 3vw, 13px)";
      bigMargin = 5;
      break;
    case "xl-mobile":
      // Mobile cover — wants the same dominance as the landscape 80px wordmark.
      big   = "clamp(52px, 15vw, 64px)";
      small = "clamp(13px, 3.6vw, 15px)";
      bigMargin = 8;
      break;
  }
  return (
    <div style={{ textAlign: align, lineHeight: 1 }}>
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="jc-v14-link"
        style={{ textDecoration: "none", color: "inherit", display: "inline-block" }}
      >
        <h1 style={{
          fontFamily: "var(--font-anton), system-ui, sans-serif",
          fontSize: big, lineHeight: 0.91, color: C.ink,
          margin: `0 0 ${bigMargin}px`, letterSpacing: "0.01em", textTransform: "uppercase",
        }}>
          PASSAGE
        </h1>
      </a>
  ...
  if (isMobile) {
    ...
    return (
      <div style={{ position: "relative", width: W, height: H, backgroundColor: C.bg, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <a href="/" title="Dramatic Adventure Theatre" className="jc-v14-imglink"
          style={{
        ...
        {/* Text panel — scrollable middle so smaller phones never clip the
            editorial framing, the headshot row, or the share controls.
            Mirrors the same pinned-photo + scrollable-middle pattern used by
            the chapter pages. */}
        <div className="jc-v14-scroll" style={{
          flex: 1, minHeight: 0, overflowY: "auto",
          display: "flex", flexDirection: "column",
          padding: `${STAMP / 2 + 8}px 22px 16px 22px`,
          textAlign: "center", alignItems: "center",
          WebkitMaskImage:
            "linear-gradient(180deg, black 0, black calc(100% - 14px), transparent 100%)",
          maskImage:
            "linear-gradient(180deg, black 0, black calc(100% - 14px), transparent 100%)",
        }}>
          <a href="/" className="jc-v14-link" style={{ textDecoration: "none" }}>
            <p style={{
              fontFamily: "var(--font-space-grotesk), system-ui, sans-serif",
              fontWeight: 700, fontSize: 9, letterSpacing: "0.28em",
              textTransform: "uppercase", color: C.pink, margin: "0 0 10px",
            }}>Dramatic Adventure Theatre</p>
          </a>
          <PassageStack size="xl-mobile" align="center" />
          <p style={{
            fontFamily: "var(--font-dm-sans), system-ui, sans-serif",
            fontSize: 10, color: C.muted, margin: "6px 0 12px",
          }}>{PROGRAM.dates}</p>

          {/* Editorial framing line — pink leader rule + italic, matching
              the desktop cover. Slightly smaller on mobile so it sits well
              between the wordmark and the pull quote. */}
          <div style={{
            display: "flex", alignItems: "center", gap: 10,
            margin: "0 0 10px", maxWidth: "92%",
          }}>
            <span style={{
            ...
            <p style={{
              ...
              color: C.ink, opacity: 0.85, margin: 0,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 14 }}>
            <a href={`/alumni/${PROFILE_SLUG}`} className="jc-v14-imglink" style={{ display: "block", textDecoration: "none", flexShrink: 0 }}>
            ...
            <div style={{ textAlign: "left", flex: 1, minWidth: 0 }}>
              <a href={`/alumni/${PROFILE_SLUG}`} className="jc-v14-link" style={{ textDecoration: "none", color: "inherit" }}>
                <p style={{
                  fontFamily: "var(--font-anton), system-ui, sans-serif",
  ...
  return (
    <div style={{ position: "relative", display: "flex", width: W, height: H, backgroundColor: C.bg, overflow: "hidden" }}>
      <div style={{
        ...
        <a href="/" className="jc-v14-link" style={{ textDecoration: "none", maxWidth: "70%" }}>
          <p style={{
            fontFamily: "var(--font-space-grotesk), system-ui, sans-serif",
        ...
        <div style={{ marginTop: 16, display: "flex", gap: 14, alignItems: "stretch" }}>
          <a href={`/alumni/${PROFILE_SLUG}`} className="jc-v14-imglink"
...
function BackCoverPage({ photos, isMobile, W, H }: { photos: Photo[]; isMobile: boolean; W: number; H: number }) {
  ...
  // V14: on mobile the back-cover stamp is sized to actually overlap the
  // surrounding mosaic squares — it should read as stamped onto the artifact,
  // not boxed in a clear zone. We scale it to a fraction of the card width
  // (clamped) so it works on narrow phones up through small tablets.
  const BACK_STAMP = isMobile
    ? Math.min(220, Math.max(180, Math.round(W * 0.58)))
    : 172;
  ...
  return (
    <div style={{ position: "relative", width: W, height: H, overflow: "hidden" }}>
      ...
      <a href="/" title="Dramatic Adventure Theatre — Home" className="jc-v14-imglink"
        style={{
      ...
      <a href="https://dramaticadventure.com/travel-opportunities"
        target="_blank" rel="noopener noreferrer" className="jc-v14-cta"
        style={{
    ...
    return (
      <div style={{ width: W, height: H, display: "flex", flexDirection: "column", backgroundColor: C.bg }}>
        ...
        <div style={{ padding: "12px 18px 6px 18px", flexShrink: 0 }}>
          <PassageStack size="lg-mobile" align="left" />
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.65rem" }}>
        ...
        <span style={{ fontFamily: "var(--font-dm-sans), system-ui, sans-serif", fontSize: "0.75rem", color: C.ink, opacity: 0.65 }}>
          /journey-card-mockup/v14 · adaptive · DAT-blue label · logo-stamp clubs · alumni-index mockups
        </span>
      </div>
      <nav style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap" }}>
        {([
          ["/journey-card-mockup/v13", "← v13"],
          ["/journey-card-mockup/v14", "v14"],
          ["/journey-card-mockup/v14/embeds", "embeds"],
          ["/journey-card-mockup/v14/alumni-index", "alumni-index"],
          ...
          <a key={href} href={href} style={{
            ...
            backgroundColor: label === "v14" ? "rgba(36,17,35,0.18)" : "rgba(36,17,35,0.09)",
          }}>{label}</a>
        ))}
      </nav>
    </div>
...
// ── Root ──────────────────────────────────────────────────────────────────────
export default function JourneyCardV14() {
  ...
  // `scale` is the CSS transform factor applied to the landscape card so it
  // fluidly fits any viewport above the mobile breakpoint. On mobile the
  // card reflows into a passport-book layout and scale stays at 1.
  const [layout, setLayout]                 = useState<{ isMobile: boolean; W: number; H: number; scale: number }>({
    isMobile: false, W: 1080, H: 580, scale: 1,
  });
  ...
  // Adaptive layout (see top-of-file strategy comment).
  //
  // Below MOBILE_BP we reflow into a passport book. Above it we keep the
  // card at its native 1080×580 and fluidly scale it with CSS transform.
  // The page-padding gutter (HORIZ_GUTTER on each side of the card) stays
  // consistent across all sizes so the card never floats in dead space.
  useEffect(() => {
    if (typeof window === "undefined") return;
    const MOBILE_BP   = 760;   // below this → passport-book reflow
    const NATIVE_W    = 1080;
    const NATIVE_H    = 580;
    const HORIZ_GUTTER = 32;   // px of breathing room left+right of the card
    const SCALE_MIN   = 0.7;   // never shrink below 70% (= 756×406)
    const SCALE_MAX   = 1.28;  // ultra-wide cap (= ~1382×742, premium feel)

    const compute = () => {
      const vw = window.innerWidth;
      if (vw < MOBILE_BP) {
        // Passport-book reflow. Width is fluid through phone sizes; height
        // follows the ≈5:8 passport aspect ratio.
        const W = Math.min(Math.max(vw - 24, 300), 460);
        const H = Math.round(W * 1.58);
        setLayout({ isMobile: true, W, H, scale: 1 });
        return;
      }
      // Landscape. Compute scale so the card always fits with the gutter
      // and feels intentional at every size.
      const available = vw - HORIZ_GUTTER * 2;
      const raw = available / NATIVE_W;
  ...
  // The visible (rendered) size of the card after CSS transform. The card
  // itself renders at native W×H and is scaled with transform-origin top-left;
  // surrounding chrome (status row, dot pager, page-label tabs) aligns to
  // these visible dimensions so the layout never floats in dead space.
  ...
  return (
    <>
      <style>{`
        .jc-v14-btn { transition: all 0.18s ease; }
        .jc-v14-btn:hover { transform: translateY(-1px); box-shadow: 0 3px 10px rgba(36,17,35,0.18); }
        .jc-v14-link { transition: opacity 0.15s ease; }
        .jc-v14-link:hover { opacity: 0.65; }
        .jc-v14-imglink { transition: transform 0.22s ease; display: block; }
        .jc-v14-imglink:hover { transform: scale(0.975); }
        .jc-v14-club { transition: background-color 0.15s ease, transform 0.18s ease; }
        .jc-v14-club:hover { background-color: rgba(36,17,35,0.08); transform: translateX(2px); }
        .jc-v14-partner { transition: background-color 0.15s ease, box-shadow 0.18s ease; }
        .jc-v14-partner:hover { background-color: #ffffff; box-shadow: 0 2px 12px rgba(36,17,35,0.10); }
        .jc-v14-cta { transition: transform 0.18s ease, box-shadow 0.18s ease; }
        .jc-v14-cta:hover { transform: translateY(-2px); box-shadow: 0 6px 22px rgba(242,51,89,0.55); }
        .jc-v14-dot:hover { background-color: rgba(36,17,35,0.45) !important; }
        /* Slim, low-contrast scrollbar for the journal scroll region. */
        .jc-v14-scroll { scrollbar-width: thin; scrollbar-color: rgba(36,17,35,0.22) transparent; }
        .jc-v14-scroll::-webkit-scrollbar { width: 4px; }
        .jc-v14-scroll::-webkit-scrollbar-track { background: transparent; }
        .jc-v14-scroll::-webkit-scrollbar-thumb {
          background: rgba(36,17,35,0.22);
          border-radius: 2px;
        }
        .jc-v14-scroll::-webkit-scrollbar-thumb:hover {
      ...
      <main style={{
        ...
        {/* Scale frame — reserves space at the *visible* (scaled) size so
            the rest of the page flows correctly. The .jc-v14-outer below
            renders at native W×H and is CSS-transformed down/up. */}
        <div style={{ width: visibleW, height: visibleH, position: "relative" }}>
          <div
            className="jc-v14-outer"
            style={{
              width: W, height: H,
          ...
          {page > 0 && (
            <button type="button" onClick={() => goTo(page - 1)} aria-label="Previous page" className="jc-v14-btn"
              style={{
          ...
          {page < TOTAL - 1 && (
            <button type="button" onClick={() => goTo(page + 1)} aria-label="Next page" className="jc-v14-btn"
              style={{
        ...
        <div style={{ width: "100%", maxWidth: visibleW }}>
        ...
        <div style={{ display: "flex", gap: 4, marginTop: 8, flexWrap: "wrap", justifyContent: "center" }}>
          {PAGE_LABELS.map((label, i) => (
            <button key={i} type="button" onClick={() => goTo(i)} className="jc-v14-btn"
              style={{
