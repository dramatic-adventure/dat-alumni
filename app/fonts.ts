...
export const dmSans = localFont({
  src: [
    // Variable TTFs are a strict glyph superset of the old per-weight
    // latin-ext WOFF2s (verified via cmap), so they're the only sources needed.
    {
...
export const spaceGrotesk = localFont({
  src: [
    // Variable TTF is a strict glyph superset of both old static sets
    // (latin-ext + v21-latin, verified via cmap) — one file covers every
    // weight 300–700, same pattern as DM Sans above.
    {
...
/* ===============================
   Program Stamp Fonts
   (randomly picked per stamp via utils/random.ts getRandomFont —
   no direct references elsewhere, but NOT dead code)
=============================== */
