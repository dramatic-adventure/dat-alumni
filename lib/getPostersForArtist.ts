...
import { productionMap, Layout, TitlePosition, Production } from "@/lib/productionMap";
...
export function getPostersForArtist(slug: string) {
  return Object.values(productionMap)
    .filter((production): production is Production => !!production.artists && slug in production.artists)
    .map((production) => {
      const layout: Layout = validLayouts.includes(production.layout as Layout)
        ? (production.layout as Layout)
        : "landscape";

      const titlePosition: TitlePosition = validTitlePositions.includes(production.titlePosition as TitlePosition)
